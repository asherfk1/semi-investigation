...
\
const PECA_LAWS = [\
  \{id:"P9",  label:"PECA S.9 \'96 Hate speech"\},\
  \{id:"P10", label:"PECA S.10 \'96 Cyberterrorism"\},\
  \{id:"P11", label:"PECA S.11 \'96 Electronic forgery"\},\
  \{id:"P20", label:"PECA S.20 \'96 Offences against dignity"\},\
  \{id:"P21", label:"PECA S.21 \'96 Offences against modesty"\},\
  \{id:"P26A",label:"PECA S.26A \'96 Fake / false information"\},\
];\
const VIOLATIONS = [\
...
function fmtDate(raw)\{ if(!raw)return"Unknown";try\{const d=new Date(typeof raw==="number"?raw*1000:raw);if(isNaN(d))return String(raw).slice(0,20);return d.toLocaleDateString("en-PK",\{day:"2-digit",month:"short",year:"numeric"\})+" \'b7 "+d.toLocaleTimeString("en-PK",\{hour:"2-digit",minute:"2-digit"\});\}catch\{return String(raw).slice(0,20);\} \}\
\
...
\
  async function startAnalysis() \{\
    ...
    const acct   = \{name, handle, platform:pf, url:profileUrl, followers:"\'97", avatar:name.split(" ").map(w=>w[0]).join("").slice(0,2).toUpperCase()\};\
    setAccount(acct); setPosts([]); setAiResults(\{\});\
    setValidated(\{\}); setRejected(\{\}); setEditViol(\{\}); setEditPeca(\{\});\
    setInvStep("loading");\
    setProgress(\{current:0, total:0, msg:"Fetching posts via Apify\'85"\});\
\
    let fetched = [];\
    try \{\
...
\
      ...
      if (!Array.isArray(items) || items.length===0) throw new Error("No posts returned \'97 profile may be private");\
\
      fetched = items.slice(0, postCount).map((item,i) => \{\
        ...
        \} else if (pf==="Facebook") \{\
          ...
          content=rawT.trim()||(mi.length>1?`[Album \'97 $\{mi.length\} images]`:mi.some(m=>m.__typename==="Video")?"[Video post]":mi.length>0?"[Image post]":"[Post with no text]");\
        ...
        \} else if (pf==="YouTube") \{\
          content=(item.title||"")+(item.description?" \'97 "+item.description.slice(0,200):"");\
...
\
    ...
    for (let i=0; i<fetched.length; i++) \{\
      const p = fetched[i];\
      setProgress(\{current:i+1, total:fetched.length, msg:`Analysing post $\{i+1\} of $\{fetched.length\} with Claude AI\'85`\});\
      try \{\
    ...
    const exhibits= vPosts.map((post,i)=>\{\
      ...
      return `<div style="border:1px solid #ddd;border-radius:8px;margin-bottom:16px;overflow:hidden">\
        <div style="background:#f5f5f5;padding:8px 14px;border-bottom:1px solid #ddd;display:flex;justify-content:space-between">\
          <b>Exhibit $\{i+1\} \'97 $\{post.type.toUpperCase()\}</b>\
          <span style="font-size:11px;color:#666">Posted: $\{post.date\} | <span style="background:$\{SEV_BG[sev]\};color:$\{SEV_COLOR[sev]\};padding:2px 8px;border-radius:20px">$\{sev.toUpperCase()\} \'b7 $\{ai.confidence\}%</span></span>\
        </div>\
        <div style="padding:12px">\
          ...
          <div style="font-size:12px;color:#185FA5;margin-bottom:6px">\uc0\u55357 \u56599  <a href="$\{post.url\}">$\{post.url\}</a></div>\
...
<div class="np"><b>SMIU Report \'97 $\{caseRef\}</b><button onclick="window.print()" style="background:#fff;color:#185FA5;border:none;border-radius:6px;padding:5px 12px;font-weight:600;cursor:pointer">Print</button></div>\
<div style="text-align:center;border-bottom:2px solid #333;padding-bottom:14px;margin-bottom:16px">\
  ...
  <div style="font-size:13px;color:#555">Social Media Account Block Request \'97 PECA 2016</div>\
...
<div style="display:grid;grid-template-columns:1fr 1fr;gap:24px;border-top:1px solid #ddd;padding-top:14px">\
  <div><div style="height:38px;border-bottom:1px solid #999;margin-bottom:5px"></div><b>$\{user?.name||"\'97"\}</b><div style="font-size:11px;color:#666">$\{user?.badge||""\}</div><div style="font-size:11px;color:#666">Date: $\{reportDate\}</div></div>\
...
<div style="margin-top:12px;text-align:center;font-size:10px;color:#aaa">$\{caseRef\} \'b7 $\{reportDate\} \'b7 SMIU \'97 CONFIDENTIAL</div>\
</body></html>`;\
...
\
  // \uc0\u9472 \u9472  IDLE \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
    ...
    <div>\
      <div style=\{s.card\}>\
        ...
        <div style=\{s.cb\}>\
          ...
          \{detectedPf && <div style=\{\{background:"#EAF3DE",border:"1px solid #C0DD97",borderRadius:7,padding:"7px 12px",fontSize:12,color:"#3B6D11",fontWeight:500,marginBottom:12\}\}>\uc0\u10003  Platform detected: <b>\{detectedPf\}</b></div>\}\
          \{!detectedPf && profileUrl.length>8 && <div style=\{\{background:"#FAEEDA",border:"1px solid #EF9F27",borderRadius:7,padding:"7px 12px",fontSize:12,color:"#633806",marginBottom:12\}\}>Unrecognised URL \'97 supported: x.com \'b7 instagram.com \'b7 facebook.com \'b7 tiktok.com \'b7 youtube.com</div>\}\
...
\
  // \uc0\u9472 \u9472  LOADING \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
...
\
  // \uc0\u9472 \u9472  REVIEW \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \u9472 \
  return (\
    <div>\
      <div style=\{\{...s.card,padding:"10px 14px",marginBottom:12\}\}>\
        <div style=\{\{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8\}\}>\
          <div>\
            <div style=\{\{fontSize:14,fontWeight:600\}\}>\{account?.name\} \'b7 \{account?.handle\}</div>\
            <div style=\{\{fontSize:11,color:"#888"\}\}>\{account?.platform\} \'b7 \{account?.url\}</div>\
          </div>\
          <div style=\{\{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center"\}\}>\
            <div style=\{\{background:"#f0f0f0",borderRadius:7,padding:"5px 10px",fontSize:11,display:"flex",gap:8\}\}>\
              <span style=\{\{color:"#1D9E75"\}\}>\uc0\u10003  \{validatedIds.length\}</span>\
              <span style=\{\{color:"#E24B4A"\}\}>\uc0\u10007  \{Object.values(rejected).filter(Boolean).length\}</span>\
              <span style=\{\{color:"#A32D2D"\}\}>\uc0\u9873  \{posts.filter(p=>(aiResults[p.id]?.violations?.length||0)>0).length\}</span>\
            </div>\
            <button onClick=\{()=>setInvStep("idle")\} style=\{btn("#f0f0f0","#333","1px solid #ccc")\}>\uc0\u8592  New scan</button>\
            \{canReport\
              ? <button onClick=\{exportReport\} style=\{btn("#1D9E75","#fff")\}>\uc0\u11015  Export report</button>\
      ...
      \{filteredPosts.map(post => \{\
        ...
        return (\
          <div key=\{post.id\} style=\{\{...s.card,border:isV?"2px solid #1D9E75":isR?"2px solid #E24B4A":"1px solid #e8e8e8"\}\}>\
            \{(isV||isR) && <div style=\{\{background:isV?"#EAF3DE":"#FCEBEB",padding:"5px 14px",borderRadius:"10px 10px 0 0",fontSize:11,fontWeight:500,color:isV?"#3B6D11":"#A32D2D"\}\}>\{isV?"\uc0\u10003  Validated \'97 included in report":"\u10007  False positive \'97 excluded"\}</div>\}\
            <div style=\{\{display:"grid",gridTemplateColumns:"270px 1fr",gap:14,padding:"12px 14px"\}\}>\
              <div>\
                <div style=\{\{fontSize:10,color:"#888",fontWeight:500,marginBottom:5\}\}>Post \'b7 \{account?.platform\}</div>\
                <div style=\{\{background:"#111",borderRadius:8,overflow:"hidden",marginBottom:6\}\}>\
                  ...
                  <div style=\{\{padding:"8px 10px"\}\}>\
                    ...
                    <div style=\{\{display:"flex",gap:12,marginTop:6,color:"#71767b",fontSize:10\}\}>\
                      <span>\uc0\u55357 \u56492 \{numFmt(post.comments)\}</span><span>\u10084 \u65039 \{numFmt(post.likes)\}</span>\
                    </div>\
                  </div>\
                </div>\
...
\
              <div style=\{\{display:"flex",flexDirection:"column",gap:8\}\}>\
                <div style=\{\{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"\}\}>\
                  ...
                  <span style=\{\{fontSize:11,color:"#888"\}\}>Confidence: <b style=\{\{color:"#111"\}\}>\{ai.confidence||"\'97"\}%</b></span>\
                ...
                \{ai.summary && <div style=\{\{background:"#f8f9fa",borderRadius:7,padding:"7px 10px",fontSize:12,lineHeight:1.5,border:"1px solid #e9ecef"\}\}><span style=\{\{fontSize:10,color:"#888",fontWeight:600\}\}>AI \'b7 </span>\{ai.summary\}</div>\}\
                ...
                <textarea style=\{\{...s.inp,minHeight:36,resize:"vertical",fontSize:11\}\} placeholder="Analyst note\'85"\
                ...
                <div style=\{\{display:"flex",gap:5,flexWrap:"wrap"\}\}>\
                  ...
                  <button onClick=\{()=>doValidate(post.id)\} style=\{btn(isV?"#1D9E75":"#f0f0f0",isV?"#fff":"#333","1px solid "+(isV?"#1D9E75":"#ccc"))\}>\{isV?"\uc0\u10003  Validated":"Validate"\}</button>\
                  <button onClick=\{()=>doReject(post.id)\}   style=\{btn(isR?"#E24B4A":"#f0f0f0",isR?"#fff":"#333","1px solid "+(isR?"#E24B4A":"#ccc"))\}>\{isR?"\uc0\u10007  Rejected":"Reject"\}</button>\
                </div>\
                \{isEx && (\
